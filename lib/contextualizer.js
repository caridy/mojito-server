...
function contextualizer(config) {
    // storing app and utilities
    contextualizer.app = this.app;
    contextualizer.merge = this.merge;
...
contextualizer.config = function (config) {
    var locals = contextualizer.app && contextualizer.app.locals;

    if (!locals) {
        throw new Error('mojito.contextualizer() should happens after mojito() statement.');
    }
